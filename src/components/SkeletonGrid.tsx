...
export function SkeletonGrid({ count, variant = "standard" }: SkeletonGridProps) {
  const isFeatured = variant === "featured";
  const items = Array.from({ length: count ?? (isFeatured ? 2 : 6) });

  return (
    <div className={isFeatured ? "grid gap-8 sm:grid-cols-2" : "grid gap-8 sm:grid-cols-2 lg:grid-cols-3"}>
      {items.map((_, i) => (
