...
export default function ImpactPage() {
  ...
  const items = useAppSelector((state) => state.impact.items);
  const loading = useAppSelector((state) => state.impact.loading);
          ...
          <>
            ...
            {highlighted.length > 0 && (
              ...
              >
                ...
                <div className="grid gap-6 sm-cols-2">
                  {highlighted.map((item, idx) => (
                    <div key={item._id}>
                      <div className="group flex flex-col items-center text-center bg-forest-600 text-white rounded-2xl p-8 shadow-lg hover-xl hover:-translate-y-1 transition-all duration-300">
                        <div className="w-16 h-16 rounded-2xl bg-white/20 flex items-center justify-center mb-6 text-white group-hover-110 transition-transform duration-300">
                          {getIcon(idx)}
                        </div>
                        <div className="text-4xl sm-5xl font-black tracking-tight mb-2">
                          {item.value}
                        </div>
            ...
            {/* All Metrics Grid */}
            <div
              className="grid gap-6 sm-cols-2 lg-cols-3"
            >
              {regular.map((item, idx) => (
                <div key={item._id}>
                  <div className="group flex flex-col items-center text-center bg-white border border-neutral-200 rounded-2xl p-6 shadow-sm hover-xl hover:-translate-y-1 transition-all duration-300">
                    <div className="w-14 h-14 rounded-2xl bg-forest-50 flex items-center justify-center mb-5 text-forest-600 group-hover-110 group-hover-forest-100 transition-all duration-300">
                      {getIcon(idx + highlighted.length)}
                    </div>
                    <div className="text-3xl sm-4xl font-bold text-neutral-900 mb-2">
                      {item.value}
                    </div>
